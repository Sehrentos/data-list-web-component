...
<article part="base"></article>`

/**
 * The ui-datalist web component
 */
export default class UIDataList extends HTMLElement {
    ...
    constructor() {
        ...
        // this has an issue, when ui-datalist is added after page init:
    ...
    connectedCallback() {
        ...
        UIDataList.datalists.push(this)

        // optional: make the ui-datalist hidden by default or use CSS: ui-datalist { display: none; }
        this.setAttribute("hidden", "")

        // bind all events
        this.addBindings()

        // optional: show warning if no input points to this ui-datalist element at all
    ...
    disconnectedCallback() {
        ...
        window.clearTimeout(UIDataList.timeoutThrottleResize)
        window.clearTimeout(UIDataList.timeoutDebounceInput)
        window.clearTimeout(UIDataList.timeoutInputFocusout)
        window.clearTimeout(UIDataList.timeoutMousedown)
        // reset previous observer if exist
        // if (this.observer != null) this.observer.disconnect()
        // remove from static collection
        UIDataList.datalists = UIDataList.datalists.filter(p => p.id != this.id)
    }
    ...
    //         // attributes: true, // monitor ui-datalist attribute changes
        ...
        this.bindings.push({
            ...
            binding: ["mousedown", (e) => {
                ...
                if (e.target.closest("section") != null) {
                    ...
                    window.clearTimeout(UIDataList.timeoutMousedown)
                    UIDataList.timeoutMousedown = window.setTimeout(() => {
                        // clear input focusout timer, that will hide this Element
                        window.clearTimeout(UIDataList.timeoutInputFocusout)
                    }, 20)
                }
    ...
    /** create option element for ui-datalist */
    ...
    createSection(value, label, textContent) {
        ...
        h2.setAttribute("part", "section-title")
        ...
        if (typeof textContent === "string" || typeof textContent === "number") {
            ...
            p.setAttribute("part", "section-content")
    ...
    static bindGlobalEvents() {
        if (UIDataList.isGlobalEventsBinded) return // only once
        UIDataList.isGlobalEventsBinded = true
        // console.log("Global DataList events are now binded")

        // bind window events
        window.addEventListener("resize", (event) => {
            if (UIDataList.timeoutThrottleResize != null) return
            UIDataList.timeoutThrottleResize = window.setTimeout(() => {
                try {
                    UIDataList.timeoutThrottleResize = null
                    ...
                    for (const target of targets) {
                        ...
                        try {
                            ...
                            UIDataList.datalists.forEach(datalist => {
        ...
        document.addEventListener("DOMContentLoaded", () => {
             ...
             * you scroll the ui-datalist with mousedown/touchstart, it begins to hide the ui-datalist.
             * to keep the ui-datalist visible in this case, we start a timeout here and check on the 
             * ui-datalist article element touchstart/mousedown event that the target element is ui-datalist section element.
            ...
            document.body.addEventListener("focusout", (e) => {
                ...
                UIDataList.timeoutInputFocusout = window.setTimeout(() => {
                    try {
                        const input = e.target.closest(`input[list]`)
                        if (input == null) return
                        UIDataList.datalists.forEach(datalist => {
            ...
            document.body.addEventListener("input", (e) => {
                ...
                window.clearTimeout(UIDataList.timeoutDebounceInput)
                UIDataList.timeoutDebounceInput = window.setTimeout(() => {
                    try {
                        const input = e.target.closest(`input[list]`)
                        if (input == null) return
                        UIDataList.datalists.forEach(datalist => {
...
// static props
UIDataList.isGlobalEventsBinded = false
UIDataList.timeoutDebounceInput = null // debounce event input to filter UI elements
UIDataList.timeoutInputFocusout = null
UIDataList.timeoutMousedown = null
UIDataList.timeoutThrottleResize = null
/**
 * keep reference to all datalists for event delegation in bindGlobalEvents
 * @type {Array<UIDataList>}
 */
UIDataList.datalists = []

// define web component
customElements.define("ui-datalist", UIDataList)